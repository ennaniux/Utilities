...
* Bash code to export from an org-file to html

When generating reports, we would like to do it via emacs org-mode. 
In this case, in the bash file we can add the following:
